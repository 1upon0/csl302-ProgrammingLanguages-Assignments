...
\\001\000\003\000\037\000\004\000\038\000\000\000\
...
\\030\000\002\000\009\000\003\000\014\000\000\000\
\\031\000\002\000\009\000\000\000\
\\032\000\000\000\
\\033\000\000\000\
\\034\000\000\000\
\\035\000\000\000\
...
\\039\000\000\000\
\\040\000\000\000\
\\041\000\005\000\023\000\000\000\
\\042\000\006\000\016\000\000\000\
\\043\000\000\000\
\\044\000\000\000\
\\045\000\000\000\
\\046\000\000\000\
\\047\000\006\000\016\000\000\000\
\\048\000\000\000\
\\049\000\000\000\
\"
val actionRowNumbers =
"\009\000\005\000\002\000\003\000\
\\004\000\010\000\008\000\018\000\
\\001\000\014\000\013\000\011\000\
\\012\000\019\000\000\000\017\000\
\\015\000\006\000\020\000\023\000\
\\024\000\001\000\022\000\000\000\
\\025\000\016\000\021\000\007\000"
val gotoT =
"\
\\001\000\027\000\002\000\006\000\003\000\005\000\004\000\004\000\
...
\\000\000\
\\000\000\
\\003\000\011\000\004\000\004\000\005\000\003\000\006\000\002\000\
...
datatype svalue = VOID | ntVOID of unit ->  unit
 ...
 | T of unit ->  (exp1 list) | TERMM of unit ->  (exp1)
 | TUPLETERM of unit ->  (exp1 list) | BODY of unit ->  (exp1)
 | HEAD of unit ->  (exp1) | LITERAL of unit ->  (exp1)
 | RULE of unit ->  (exp1) | FACT of unit ->  (exp1)
 | STATEMENT of unit ->  (exp1) | PROGRAM of unit ->  (exp1 list)
 | START of unit ->  (exp1)
end
...
|  ( 4, ( ( _, ( _, _, PERIOD1right)) :: ( _, ( MlyValue.PROGRAM 
PROGRAM1, PROGRAM1left, _)) :: rest671)) => let val  result = 
MlyValue.PROGRAM (fn _ => let val  (PROGRAM as PROGRAM1) = PROGRAM1 ()
 in (PROGRAM)
end)
 in ( LrTable.NT 1, ( result, PROGRAM1left, PERIOD1right), rest671)

end
|  ( 5, ( ( _, ( _, _, PERIOD1right)) :: ( _, ( MlyValue.FACT FACT1, 
...
|  ( 6, ( ( _, ( _, _, PERIOD1right)) :: ( _, ( MlyValue.RULE RULE1, 
...
|  ( 8, ( ( _, ( MlyValue.LITERAL LITERAL1, LITERAL1left, 
...
|  ( 9, ( ( _, ( MlyValue.LITERAL LITERAL1, LITERAL1left, 
...
|  ( 10, ( ( _, ( MlyValue.LITERAL LITERAL1, _, LITERAL1right)) :: _
 :: ( _, ( MlyValue.BODY BODY1, BODY1left, _)) :: rest671)) => let
 val  result = MlyValue.BODY (fn _ => let val  (BODY as BODY1) = BODY1
 ()
 val  (LITERAL as LITERAL1) = LITERAL1 ()
 in (node ("commaa",[BODY,LITERAL]))
end)
 in ( LrTable.NT 7, ( result, BODY1left, LITERAL1right), rest671)
end
|  ( 11, ( ( _, ( MlyValue.BODY BODY1, _, BODY1right)) :: _ :: ( _, ( 
...
|  ( 13, ( ( _, ( MlyValue.TUPLETERM TUPLETERM1, _, TUPLETERM1right))
...
NAME1 ()
 ...
 in (node (NAME,TUPLETERM))
end)
 in ( LrTable.NT 5, ( result, NAME1left, TUPLETERM1right), rest671)

end
|  ( 14, ( ( _, ( MlyValue.TERMM TERMM1, TERMM1left, TERMM1right)) :: 
rest671)) => let val  result = MlyValue.T (fn _ => let val  (TERMM as 
TERMM1) = TERMM1 ()
 in ( [TERMM])
end)
 in ( LrTable.NT 10, ( result, TERMM1left, TERMM1right), rest671)
end
|  ( 15, ( ( _, ( MlyValue.TERMM TERMM1, _, TERMM1right)) :: _ :: ( _,
 ( MlyValue.T T1, T1left, _)) :: rest671)) => let val  result = 
MlyValue.T (fn _ => let val  (T as T1) = T1 ()
 val  (TERMM as TERMM1) = TERMM1 ()
 in (T @ [TERMM])
end)
 in ( LrTable.NT 10, ( result, T1left, TERMM1right), rest671)
end
|  ( 16, ( ( _, ( _, _, RPAREN1right)) :: ( _, ( MlyValue.T T1, _, _))
 :: ( _, ( _, LPAREN1left, _)) :: rest671)) => let val  result = 
MlyValue.TUPLETERM (fn _ => let val  (T as T1) = T1 ()
 in ( T )
end)
 in ( LrTable.NT 8, ( result, LPAREN1left, RPAREN1right), rest671)
end
|  ( 17, ( ( _, ( MlyValue.NAME NAME1, NAME1left, NAME1right)) :: 
rest671)) => let val  result = MlyValue.TERMM (fn _ => let val  (NAME
 as NAME1) = NAME1 ()
 in (leaf NAME)
end)
...
) => let val  result = MlyValue.TERMM (fn _ => let val  (VAR as VAR1)
 = VAR1 ()
 in (leaf VAR)
end)
 in ( LrTable.NT 9, ( result, VAR1left, VAR1right), rest671)
end
|  ( 19, ( ( _, ( MlyValue.TUPLETERM TUPLETERM1, _, TUPLETERM1right))
 :: ( _, ( MlyValue.NAME NAME1, NAME1left, _)) :: rest671)) => let
 val  result = MlyValue.TERMM (fn _ => let val  (NAME as NAME1) = 
NAME1 ()
 val  (TUPLETERM as TUPLETERM1) = TUPLETERM1 ()
 in (node (NAME,TUPLETERM))
end)
 in ( LrTable.NT 9, ( result, NAME1left, TUPLETERM1right), rest671)
